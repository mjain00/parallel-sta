...
void DAG::createTaskGraph(const ASIC& asic) {
    ...
    for (int outputNode : asic.outputs) {
        ...
        while (!q.empty()) {
            ...
            std::cout << "Processing node " << current << ", looking at its fanins..." << std::endl;
            for (int fanin : reverseAdjList[current]) { // reverse adjacency list!
                std::cout << "  - Fanin: " << fanin << std::endl; // Debug print for fanins
                ...
                if (!visited.count(fanin)) {
                    ...
                    // Add fanin to the queue for further processing
                    q.push(fanin);
                    visited.insert(fanin);
                } else {
                    std::cout << "    Fanin " << fanin << " already visited." << std::endl;
                }
            }
    
            // If queue is empty, try decrementing current and checking if it's in reverseAdjList
            if (q.empty()) {
                int next = current - 1;
                std::cout << "Queue is empty. Decrementing current: " << current << " to " << next << std::endl;
    
                // Check if next exists in reverseAdjList
                if (reverseAdjList.find(next) != reverseAdjList.end()) {
                    std::cout << "Node " << next << " exists in reverseAdjList. Adding to queue." << std::endl;
    
                    // Add the next node to the queue
                    q.push(next);
                    visited.insert(next);
                } else {
                    std::cout << "Node " << next << " not found in reverseAdjList. No further processing." << std::endl;
                }
            }
        }
    }
...
void DAG::propagateBeRequired(const Cell& current_cell, int current_id, int fanin_id, const ASIC& asic, const std::map<int, Cell>& cell_map) {
    // Retrieve the current required time for the current cell
    ...
    // Critical section: Only the update part is protected
    #pragma omp critical
    {
    ...
    // Log the update (optional, based on verbosity)
    if (verbose) {
