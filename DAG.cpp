...
std::unordered_map<int, float> DAG::analyzeTiming(const ASIC& asic, const std::map<int, Cell>& cell_map, std::vector<int> &sorted) {
    ...
    // === STEP 1: Parallel Output Initialization ===
#pragma omp parallel for
    for (int i = 0; i < asic.outputs.size(); ++i) {
        int output = asic.outputs[i];
#pragma omp critical
    {
    ...
    // === STEP 2: Sequential Backward Propagation ===
    for (auto it = sorted.rbegin(); it != sorted.rend(); ++it) {
        int current = *it;
    ...
    // === STEP 3: Parallel Slack Computation ===
#pragma omp parallel for
